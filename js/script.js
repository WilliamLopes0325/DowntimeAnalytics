...
function filtrarPorDataETurnoEArea(
    ...
    // Converte os inputs do filtro salvando apenas o início e o fim do dia local
    const inicio = new Date(`${dataInicial}T00:00:00`);
    const fim = new Date(`${dataFinal}T23:59:59.999`);

    // Função interna para converter o formato "DD/MM/AAAA" da planilha
function converterDataPlanilha(dataStr) {
    ...
    return dados.filter(item => {
        // Usa a conversão correta para a coluna Date da sua planilha
        const dataRegistro = converterDataPlanilha(item.Date);

        // Se a data for inválida por algum erro na planilha, ignora o item
